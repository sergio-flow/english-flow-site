import countryMapping, { CountryMapping } from "@/_utils/countryMapping";
...
export default function Hero(params: Params) {
    ...
    const country = countryMapping
        .find((country: CountryMapping) => country.countryCode.toLowerCase() === countryCode.toLowerCase())

    return (
        <div className="w-full xl:w-[1340px] mx-auto px-4 py-6 gap-10 mb-6">
            {country && (
                <div className="country-language flex items-center gap-4 mb-10">
                    <Link href={`/`} className="flex cursor-pointer items-center gap-2 p-1 px-2 pr-3 rounded-lg bg-white/10">
                        <span className={`em em-${countryCode.toLowerCase() === 'id' ? 'indonesia' : countryCode.toLowerCase()} em-flag-${country.countryCode.toLowerCase()}`} />
                        <span className="text-white font-semibold text-xs">{country.countryName}</span>
                    </Link>
                    <span className="text-white text-lg">→</span>
                    <div className="flex items-center rounded-lg bg-white/10">
                        {Object.keys(availableLanguages).map((key, index) => (
                            <Link key={key} href={`/${countryCode}/${key.toLowerCase()}`} className={`${index === 0 ? "rounded-l-lg" : ""} ${index === Object.keys(availableLanguages).length - 1 ? "rounded-r-lg" : ""} cursor-pointer text-white py-1 px-2 font-semibold text-xs ${key.toLowerCase() === languageCode.toLowerCase() ? 'bg-orange-400' : ''}`}>{availableLanguages[key]}</Link>
                        ))}
                    </div>
                </div>
            )}
